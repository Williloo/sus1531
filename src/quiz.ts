...
export function adminQuizList( userId: number ): Error | { quizzes: QuizDetails[] } {
  ...
  let userQuizzes: Quiz[] = store.quizzes.filter(quiz => quiz.creatorId === userId)
  let res: QuizDetails[] = userQuizzes.map(
 ...
 * @returns { Object } - Empty object
 */
export function adminQuizCreate( userId: number, name: string, description: string ): Error | { quizId: number } {
  let store: Data = getData()
 ...
 * @returns { Object } - Empty object
 */
export function adminQuizRemove( userId: number, quizId: number ): Error | EmptyObject {
  let store: Data = getData()
  ...
  let quiz: null | Quiz = findQuiz(userId, quizId, store.quizzes)
  if (!quiz) {
 ...
 * @returns { Object } - Empty object
 */
export function adminQuizNameUpdate( userId: number, quizId: number, name: string ): Error | EmptyObject {
  let store: Data = getData()
 ...
 * @returns { Object }
 */
export function adminQuizDescriptionUpdate( userId: number, quizId: number, description: string ): Error | EmptyObject {
  let store: Data = getData()
