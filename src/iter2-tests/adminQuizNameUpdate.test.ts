import {
  ...
  describe('tests for adminQuizNameUpdate', () => {
    ...
    beforeEach(() => {
      clear()
      sessionToken = adminAuthRegister('hayden.smith@unsw.edu.au', 'myPassword1', 'Hayden', 'Smith').session
    ...
    describe('Success cases', () => {
      ...
      test('same name can be used by different users', () => {
        const otherUserResult = adminAuthRegister('other.user@unsw.edu.au', 'password123', 'Other', 'User')
        const otherSessionToken = otherUserResult.session
        const otherQuizId = adminQuizCreate(otherSessionToken, 'Other Quiz', 'Other description').quizId
    ...
    describe('Error cases', () => {
      ...
      test('error when quiz is not owned by user', () => {
        const newUserResult = adminAuthRegister('another.user@unsw.edu.au', 'anotherPassword1', 'Another', 'User')
      ...
      test('error when name is already used by the current logged in user for another quiz', () => {
        const secondQuizId = adminQuizCreate(sessionToken, 'Second Quiz', 'Another quiz description').quizId
