  ...
  describe('PUT /v1/admin/user/details', () => {
    ...
    describe('Success cases', () => {
      ...
      test('Can update to same email (no change)', () => {
        expect(adminUserDetailsUpdate(sessionToken, 'user@example.com', 'Updated', 'Name')).toEqual({})
      ...
      test('Successful update with valid special characters in names', () => {
        expect(adminUserDetailsUpdate(sessionToken, 'updated@example.com', "O'Connor", 'Smith-Jones')).toEqual({})
        const res = adminUserDetails(sessionToken)
    ...
    describe('Error cases', () => {
      ...
      test('Error when first name is invalid with invalid characters', () => {
        const res = adminUserDetailsUpdate(sessionToken, 'updated@example.com', 'Invalid@Name', 'Name')
      ...
      test('Error when last name is invalid', () => {
        const res = adminUserDetailsUpdate(sessionToken, 'updated@example.com', 'Updated', 'Invalid@Name')
        expect(res).toStrictEqual(400)
        const userDetails = adminUserDetails(sessionToken)
        expect(userDetails.user.name).toStrictEqual('John Doe')
      })
      ...
      test('Error when last name is too long', () => {
        const res = adminUserDetailsUpdate(sessionToken,'updated@example.com','Updated','ThisLastNameIsMuchTooLongForTheSystem')
        expect(res).toStrictEqual(400)
        const userDetails = adminUserDetails(sessionToken)
        expect(userDetails.user.name).toStrictEqual('John Doe')
      })
